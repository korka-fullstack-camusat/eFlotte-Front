...
import { Plus, X, Upload, FileSpreadsheet, CheckCircle2, AlertTriangle, Filter, Settings, Pencil, Trash2, Car, Search, ListOrdered, Users, MapPin, BarChart2 } from "lucide-react";
...
import type { MissionChauffeur, FiltresMissions, MissionsFilters, ImportMissionsResult } from "@/types";
...
export default function ChauffeurPolesPage() {
  ...
  const [result, setResult] = useState<ImportMissionsResult | null>(null);
  ...
  const openCharts = () => {
    ...
    missionChauffeurService.getAll({ ...filters, page: 1, page_size: 500 })
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setResult(null);
    try {
      const res = await missionChauffeurService.importExcel(file);
      setResult(res);
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
              <div className="bg-camublue-900 px-6 py-4 flex items-center justify-between sticky top-0">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-white font-bold text-sm">Graphiques — Chauffeurs Pôles</p>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="bg-camublue-900 px-6 py-4 flex items-center justify-between sticky top-0">
              <div className="flex items-center gap-3">
                ...
                <p className="text-white font-bold text-sm">Importer CHAUFFEUR POLES (Excel)</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-gray-600">
                Sélectionnez le fichier Excel contenant la feuille <strong>CHAUFFEUR POLES</strong> (colonnes : DATE, IMMA, CHAUFFEUR, DEMANDEUR, TELEPHONE, PROJET, DESTINATION, DATE DEPART, DATE RETOUR, COMMENTAIRES).
              </p>
