...
import { Plus, X, Upload, FileSpreadsheet, CheckCircle2, AlertTriangle, Filter, Settings, Pencil, Trash2, FileText, Search, ListOrdered, Building2, ClipboardList, BarChart2 } from "lucide-react";
import {
...
import type { SuiviDevis, FiltresDevis, DevisFilters, ImportDevisResult } from "@/types";
...
export default function SuiviDevisPage() {
  ...
  const [result, setResult] = useState<ImportDevisResult | null>(null);
  ...
  const openCharts = () => {
    ...
    suiviDevisService.getAll({ page: 1, page_size: 500 })
      .then(res => setChartItems(res.items))
      .catch(() => {})
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setResult(null);
    try {
      const res = await suiviDevisService.importExcel(file);
      setResult(res);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="bg-camublue-900 px-6 py-4 flex items-center justify-between sticky top-0">
              <div className="flex items-center gap-3">
                ...
                <p className="text-white font-bold text-sm">Importer SUIVI DES DEVIS (Excel)</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-gray-600">
                Sélectionnez le fichier Excel contenant la feuille <strong>SUIVI DES DEVIS</strong> (colonnes : DESCRIPTIONS, # N° DEVIS, VALEUR DEVIS, DATE, MONTANT, SOUS-TRAITANT/SUPPLIERS, MATRICULE, CODE SNC, PO EMIS).
              </p>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
              ...
              <div className="bg-camublue-900 px-6 py-4 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-bold text-sm">Statistiques — Suivi des Devis</p>
                    <p className="text-white/70 text-xs">{chartItems.length} devis chargés</p>
                  </div>
                </div>
