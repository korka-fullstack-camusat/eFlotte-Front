  ...
  return (
    <AppLayout>
      <div className="max-w-4xl mx-auto">
        {/* En-tête */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-camublue-900">Import global</h1>
                    ...
                    return (
                      <tr key={s.key} className="hover:bg-gray-50/60">
                        ...
                        <td className="px-4 py-3 text-center">
                          {isSkipped ? "—" : hasErrors ? (
                            <span className="font-semibold text-red-600">{r.errors.length}</span>
                          ) : (
            ...
            {totalErrors > 0 && (
              <div className="bg-red-50 border border-red-100 rounded-2xl p-5 mb-6">
                <p className="text-sm font-semibold text-red-700 mb-3 flex items-center gap-2">
                  <AlertTriangle size={15} /> Détail des lignes ignorées
                </p>
            ...
            {SECTIONS.some(s => result[s.key].skipped) && (
              <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5 mb-6">
                ...
                <ul className="text-xs text-gray-500 list-disc pl-4 space-y-1">
                  {SECTIONS.filter(s => result[s.key].skipped).map(s => (
                    <li key={s.key}><strong>{s.label}</strong> ({s.sheet}) — {result[s.key].skip_reason}</li>
                  ))}
                </ul>
              </div>
            )}
                  ...
                  {history.map(entry => (
                    <>
                      <tr key={entry.id} className="hover:bg-gray-50/60">
                        ...
                        <td className="px-4 py-3 text-xs text-gray-600 max-w-[200px] truncate" title={entry.filename ?? ""}>
                          {entry.filename ?? "—"}
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-600">{entry.username ?? "—"}</td>
                        <td className="px-4 py-3 text-center font-semibold text-emerald-600 text-xs">
                          {entry.total_created.toLocaleString("fr-FR")}
                        </td>
                        <td className="px-4 py-3 text-center font-semibold text-amber-600 text-xs">
                          {entry.total_updated.toLocaleString("fr-FR")}
                        </td>
                        <td className="px-4 py-3 text-center text-xs">
                          {entry.total_errors > 0
                            ? <span className="font-semibold text-red-600">{entry.total_errors}</span>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            ...
                            className="text-gray-400 hover:text-camublue-900 transition"
                          >
