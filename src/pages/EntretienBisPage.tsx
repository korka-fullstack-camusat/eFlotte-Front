...
export default function EntretienBisPage() {
  ...
  const [filters, setFilters] = useState<{ rt?: string; statut?: string; modele?: string }>({});
  ...
  const optRts     = [...new Set(entretiens.map(e => e.rt).filter(Boolean))] as string[];
  const optStatuts = [...new Set(entretiens.map(e => e.statut).filter(Boolean))] as string[];
  const optModeles = [...new Set(entretiens.map(e => e.modele).filter(Boolean))] as string[];
  ...
  // Vidange effectuée = au moins 1 palier renseigné ; en cours = aucun palier
  const hasVidange = (e: EntretienBis) =>
    Object.values(e.paliers ?? {}).some(v => v !== null && v !== undefined);
  ...
  const filtered = entretiens.filter(e => {
    ...
    if (vidangeFilter === "effectuee" && !hasVidange(e)) return false;
    if (vidangeFilter === "en_cours"  &&  hasVidange(e)) return false;
  ...
  const nbEffectuee = entretiens.filter(hasVidange).length;
  const nbEnCours   = entretiens.filter(e => !hasVidange(e)).length;
  ...
  return (
    <AppLayout>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        ...
        <KpiCard label="Modèles distincts" value={optModeles.length} icon={<AlertTriangle size={20} />} bg="bg-amber-100" text="text-amber-600" />
        <KpiCard label="Statuts distincts" value={optStatuts.length} icon={<ShieldCheck size={20} />} bg="bg-emerald-100" text="text-emerald-600" />
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setFilterModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">RT</label>
                <select value={draft.rt ?? ""} onChange={e => setDraft(d => ({ ...d, rt: e.target.value || undefined }))} className="input-base">
                  <option value="">Tous</option>
                  {optRts.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Statut</label>
                <select value={draft.statut ?? ""} onChange={e => setDraft(d => ({ ...d, statut: e.target.value || undefined }))} className="input-base">
                  <option value="">Tous les statuts</option>
                  {optStatuts.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Modèle</label>
                <select value={draft.modele ?? ""} onChange={e => setDraft(d => ({ ...d, modele: e.target.value || undefined }))} className="input-base">
                  <option value="">Tous les modèles</option>
                  {optModeles.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
      ...
      {/* ══ Modal Graphiques ══════════════════════════════════════════════ */}
      {showCharts && (() => {
        const COLORS = ["#1e3a5f","#2a5298","#3b6fc4","#5b8de0","#7aaee8","#9ec5f0","#b8d4f5","#d0e5fa"];

        // Répartition par modèle
        const parModele: Record<string, number> = {};
        entretiens.forEach(e => { const k = e.modele || "—"; parModele[k] = (parModele[k] || 0) + 1; });
        const modeleData = Object.entries(parModele).sort((a, b) => b[1] - a[1]).map(([name, value]) => ({ name, value }));
                  ...
                  <div className="bg-gray-50 rounded-2xl p-4">
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={paliersData} layout="vertical" margin={{ left: 8, right: 20 }}>
                        ...
                        <Bar dataKey="value" name="Paliers" radius={[0, 4, 4, 0]}>
